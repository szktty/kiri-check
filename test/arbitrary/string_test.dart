...
void main() {
  group('string', () {
    ...
    group('character set (ascii)', () {
      property('alphanum', () {
        testForAll(string(characterSet: alphanum), (value) {
          expect(value, matches(RegExp(r'^[a-zA-Z0-9]*$')));
        });
      });

      property('decimal digit', () {
        testForAll(string(characterSet: digit), (value) {
          expect(value, matches(RegExp(r'^[0-9]*$')));
        });
      });

      property('hex digit', () {
        testForAll(string(characterSet: CharacterSet.hexDigit()), (value) {
          expect(value, matches(RegExp(r'^[0-9a-fA-F]*$')));
        });
      });

      property('octal digit', () {
        testForAll(string(characterSet: CharacterSet.octalDigit()), (value) {
          expect(value, matches(RegExp(r'^[0-7]*$')));
        });
      });

      property('letter', () {
        testForAll(string(characterSet: letter), (value) {
          expect(value, matches(RegExp(r'^[a-zA-Z]*$')));
        });
      });

      property('upper', () {
        testForAll(string(characterSet: upper), (value) {
          expect(value, matches(RegExp(r'^[A-Z]*$')));
        });
      });

      property('lower', () {
        testForAll(string(characterSet: lower), (value) {
          expect(value, matches(RegExp(r'^[a-z]*$')));
        });
      });

      property('symbol', () {
        testForAll(string(characterSet: symbol), (value) {
          expect(value, matches(RegExp(r'^[!-/:-@[-`{-~]*$')));
        });
      });

      property('whitespaces', () {
        testForAll(string(characterSet: whitespaces), (value) {
          expect(value, matches(RegExp(r'^[\s]*$')));
        });
      });

      property('newlines', () {
        testForAll(string(characterSet: newlines), (value) {
          expect(value, matches(RegExp(r'^[\n\r]*$')));
        });
      });

      property('whitespacesAndNewlines', () {
        testForAll(string(characterSet: whitespacesAndNewlines), (value) {
          expect(value, matches(RegExp(r'^[\s\n\r]*$')));
        });
      });
    });
    ...
    group('generation', () {
      property('min', () {
        const min = 10;
        testForAll(string(minLength: min, characterSet: alphanum), (value) {
      ...
      property('max', () {
        const max = 10;
        testForAll(string(maxLength: max, characterSet: alphanum), (value) {
      ...
      property('min, max', () {
        ...
        const max = 20;
        testForAll(
            string(minLength: min, maxLength: max, characterSet: alphanum),
            (value) {
          expect(value.length, greaterThanOrEqualTo(min));
          expect(value.length, lessThanOrEqualTo(max));
        });
...
void testGeneratingUnicode({
  ...
  testForAll(
    string(minLength: 100, maxLength: 100, characterSet: target),
    (value) {
